...
FilePath: /flask_restxtra_fluffy/examples/app/managers/role_manager/parameters.py
"""
...
class GetRolesInfoParameters(QueryParameters):
    ...
    order_prop: str = String(
        validate=validate.OneOf(choices=set(RolesProfileSchema().fields.keys())),
    ...
    @post_load
    def process_get_roles_info(self, data: "GetRolesInfoParameters", **_kwargs) -> dict:
        """query roles info"""
        return {
...
class CreateRoleParameters(JSONParameters):
    ...
    @post_load
    def process_create_role(self, data: "CreateRoleParameters", **_kwargs) -> dict:
        """create a new role"""
...
class UpdateRoleParameters(JSONParameters):
    ...
    @validates_schema
    def validate(self, data: "UpdateRoleParameters", **_kwargs) -> Optional[NoReturn]:
        """validate schema"""
        if all(value is None for key, value in data.items() if key != "role_id"):
            raise ValidationError("At least one field should be provided")
        if data["routes"] is not None and not set(data["routes"]) < Routes.get_all_names():
...
class DeleteRoleParameters(JSONParameters):
    ...
    role_id: uuid.UUID = UUID(required=True, metadata={"description": "id of the role to be deleted"})
